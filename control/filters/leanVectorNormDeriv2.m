...
n_dt2(3) = leanVectorDeriv2Comp(nn,nn_dt,nn_dt2,'z');
    ...
    function n_comp_dt2 = leanVectorDeriv2Comp(nn,nn_dt,nn_dt2,comp)
        ...
        xyz = x^2+y^2+z^2;
        n_comp_dt2 = divideFinite(xss,sqrt(xyz)) ...
            -divideFinite(xs*(2*x*xs+2*y*ys+2*z*zs),xyz^(3/2)) ...
            +x*(...
                divideFinite(3*(2*x*xs+2*y*ys+2*z*zs)^2,4*xyz^(5/2)) ...
                -divideFinite(2*x*xss+2*xs^2+2*y*yss+2*ys^2+2*z*zss+2*zs^2,2*xyz^(3/2)) ...
            );
    end
end