function [int_omega, omega, dot_omega] = ...
    quatAttitudeControllerFullStateFastApprox...
    (q_bg, omega_Kb, dot_omega_Kb, q_cmd, dot_q_cmd, ddot_q_cmd)
...
% aircraft with low dynamics or simulations.
...
%% Velocity Part
omega_cmd_q = 2 * quatMultiply(quatConj(q_cmd), dot_q_cmd);
omega_cmd = omega_cmd_q(2:4);
omega = omega_cmd - omega_Kb;

%% Acceleration Part
dot_omega_cmd_q = 2 * (quatMultiply(quatConj(q_cmd), ddot_q_cmd) ...
                  + quatMultiply(quatConj(dot_q_cmd), dot_q_cmd));
dot_omega_cmd = dot_omega_cmd_q(2:4);
