function apPar_modifyCodeExportFiles(ap_pars, code_pars, var_infos, top_vars)
...
% If parameters are static, make them non-static (else, initialization will fail)
were_params_static = false;
for i = length(top_vars.type)
    expression = ['(?<=\n) *static\s+(?=',top_vars.type{i},'\s+',top_vars.name{i},')'];
    f_mod_tmp = regexprep(f_mod, expression, '  ');
end
if ~isequal(f_mod_tmp,f_mod)
    were_params_static = true;
    f_mod = f_mod_tmp;
end
...
if were_params_static
    ...
    for i = length(top_vars.type)
        expression = ['(?:\r?\n[ \t]*//[^\r\n]*)*' ...
            '\r?\n[ \t]*',top_vars.type{i},'\s+',code_pars.class_name, ...
            '::',top_vars.name{i},'\s*=\s*\{[\s\S]*?\r?\n\};\s*\r?\n?'];
        f_mod = regexprep(f_mod, expression, '\r\n');
    end

    % Override header file
    fid = fopen(code_pars.data_file, 'w');
    fprintf(fid, '%s', f_mod);
    fclose(fid);
end
