...
% 
% Inputs:
%   alpha_deg      	angle of attack (Nx1 array), in deg
%   Ma             	Mach number (Nx1 array), dimensionless
%   c_L            	lift coefficient (Nx1 array), dimensionless
%   c_D           	drag coefficient (Nx1 array), dimensionless
...
%                 	the lift coefficient (1x6 array or Nx6 array):
%                       fcl(:,1):  	alpha_0
...
%                 	the drag coefficient (1x6 array or Nx6 array):
...
%                  	the pitching moment coefficient (1x5 array or Nx5
%                  	array):
...
if alpha_min > -1
    ...
    n_dupl = 300;
    alpha_deg_cl_ext2 = [ repmat(alpha_deg_cl_ext1(2:4),n_dupl,1); alpha_deg_cl_ext1 ];
    c_l_ext2 = [ zeros(3*n_dupl,1); c_l_ext1 ];
    c_l_ext2(1:3*n_dupl) = interp1( alpha_deg_cl_ext1, c_l_ext1, alpha_deg_cl_ext2(1:3*n_dupl), 'makima' );
    
    n_dupl = 30;
    alpha_deg_cl_ext3 = [ repmat(alpha_deg_cl_ext1([1,5]),n_dupl,1); alpha_deg_cl_ext1 ];
    c_l_ext3 = [ zeros(2*n_dupl,1); c_l_ext1 ];
