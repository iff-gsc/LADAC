...
% if strcmp(wing.config.method,'DLM')
%     wing.state.geometry.vortex.pos(3,:,end) = wing.geometry.vortex.pos(3,:,end) + ...
%         diff(wing.state.geometry.vortex.pos(1,:,end-1:end),1,3) ...
%         .* sind( ...
%             interp1(wing.state.geometry.ctrl_pt.pos(2,:,end),-wing.state.actuators.segments.pos(1,:),wing.state.geometry.vortex.pos(2,:,end), 'linear', 'extrap' ) ...
%             );
% end
